...
class CashController extends Controller
{
	...
	public function BankIndex()
    {
		...
		foreach($banks as $k=>$val)
		{
			...
			$totalTransaction =  DB::table('bank_tran')
							->select(DB::raw('SUM(bank_tran.tran_amt) as totalTransaction'))
							->where('bankId', '=', $val->id)
							->get();
    ...
    public function BankStatement($bankId)
    {
		...
		if(Auth::user()->u_type ==1){ //ca
			$bank_tran =  DB::table('bank_tran')
							->select(DB::raw('bank_tran.*,company_profiles.comp_name,ca_assigns.ca_id'))
							->leftJoin('company_profiles', 'bank_tran.added_by', '=', 'company_profiles.userId')
							->leftJoin('ca_assigns', 'bank_tran.added_by', '=', 'ca_assigns.comp_id')
							->where('ca_assigns.ca_id','=',$userId)
							->where('ca_assigns.ca_assign_status','=',1)
							->where('bank_tran.bankId','=',$bankId)
							->orderBy('id', 'DESC')->paginate(10);
		}elseif(Auth::user()->u_type ==2){ //user
			$bank_tran =  DB::table('bank_tran')
							->select(DB::raw('bank_tran.*,company_profiles.comp_name'))
							->leftJoin('company_profiles', 'bank_tran.added_by', '=', 'company_profiles.userId')
							->where('bank_tran.added_by', '=', $userId)
							->where('bank_tran.bankId','=',$bankId)
							->orderBy('bank_tran.id', 'DESC')->paginate(10);
		}
		elseif(Auth::user()->u_type ==3){ //admin
			$bank_tran =  DB::table('bank_tran')
							->select(DB::raw('bank_tran.*,company_profiles.comp_name'))
		...
		$bank_tran_pagination = $bank_tran;
		
		$array = array();
		foreach($bank_tran as $k=>$val)
		{
			$array[$val->id]['id'] = $val->id;
		...
		$bank_tran = json_decode(json_encode($array));
        ...
        return view('pages.bank-statement')->with([
			...
			'bank_tran' => $bank_tran,
			'bankId' => $bankId,
			'bank_tran_pagination' => $bank_tran_pagination,
